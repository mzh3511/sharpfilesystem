    ...
    public class TypeDictionary<T> : ITypeDictionary<T>, IServiceProvider
    {
        ...
        private void ValidateType(Type type)
        {
            if (!GetBaseType().IsAssignableFrom(type))
                throw new ArgumentException("The specified type is not a subtype of '" + GetBaseType() + "'.",
                    nameof(type));
        }
        ...
        public IEnumerable<T> GetExplicit(Type type)
        {
            if (type.IsAbstract)
                throw new ArgumentException(
                    "The specified type is not a instantiatable type and cannot be explicitly returned.", nameof(type));
            ValidateType(type);
            ICollection<T> itemsOfType;
            if (types.TryGetValue(type, out itemsOfType))
                foreach (var item in itemsOfType)
